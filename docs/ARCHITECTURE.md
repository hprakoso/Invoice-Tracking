...
`GET /api/dashboard/export` builds an `.xlsx` workbook on demand with `exceljs`: a single "Invoices" sheet matching the active filters. Streamed directly in the response, nothing persisted to disk. It carried a second "KPI Summary" sheet until 2026-09-14.
