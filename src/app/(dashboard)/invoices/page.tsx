    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">{invoices.length} invoice ditemukan</p>
        </div>
        {canUpload && (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50">
                <th className="text-left px-4 py-3 text-xs text-gray-500 font-medium whitespace-nowrap">No. Invoice</th>
                <th className="text-left px-4 py-3 text-xs text-gray-500 font-medium">Vendor</th>
                <th className="text-left px-4 py-3 text-xs text-gray-500 font-medium hidden md:table-cell whitespace-nowrap">Tgl Invoice</th>
                <th className="text-left px-4 py-3 text-xs text-gray-500 font-medium hidden sm:table-cell whitespace-nowrap">Jatuh Tempo</th>
                <th className="text-right px-4 py-3 text-xs text-gray-500 font-medium whitespace-nowrap">Total</th>
                ...
                invoices.map((inv, i) => (
                  <motion.tr
                    ...
                    className={`border-b last:border-0 hover:bg-blue-50/50 cursor-pointer transition-colors ${
                      isOverdue(inv.dueDate, inv.status) ? 'bg-red-50/30' : ''
                    }`}
                  >
                    ...
                    <td className="px-2 py-3"><ChevronRight className="h-4 w-4 text-gray-300" /></td>
                  </motion.tr>
                ))
              )}
            </tbody>
